...
export default function LandingPage() {
  ...
  return (
    <div className="relative min-h-screen w-full flex flex-col justify-between overflow-hidden bg-black text-white">
      ...
      <header className="relative z-50 flex items-center justify-between p-6 w-full max-w-7xl mx-auto">
        <Link href="/">
          <img src="https://api.frameclubbilardo.com/assets/logo.svg" alt="Frame Bilardo" className="h-10 md:h-14 object-contain drop-shadow-[0_0_15px_rgba(255,255,255,0.2)] relative z-50" />
        </Link>
        ...
        <button
          ...
          className="lg:hidden p-2 text-white hover:text-emerald-400 transition-colors z-50 relative"
        >
        ...
        {isMobileMenuOpen && (
          <div className="absolute top-0 left-0 w-full h-screen bg-black/95 backdrop-blur-xl z-40 flex flex-col items-center justify-center space-y-8 animate-in fade-in zoom-in-95 duration-300">
            <Link onClick={() => setIsMobileMenuOpen(false)} href="/hakkimizda" className="text-2xl font-bold tracking-wider uppercase hover:text-emerald-400 transition-colors">Hakkımızda</Link>
            <Link onClick={() => setIsMobileMenuOpen(false)} href="/galeri" className="text-2xl font-bold tracking-wider uppercase hover:text-emerald-400 transition-colors">Galeri</Link>
            <Link onClick={() => setIsMobileMenuOpen(false)} href="/iletisim" className="text-2xl font-bold tracking-wider uppercase hover:text-emerald-400 transition-colors">Bize Ulaşın</Link>
            <Link onClick={() => setIsMobileMenuOpen(false)} href="/reservation" className="text-2xl font-bold tracking-wider uppercase text-emerald-400 border border-emerald-500/30 bg-emerald-500/10 px-6 py-2 rounded-full">Rezervasyon Yap</Link>
          </div>
        )}
      </header>
